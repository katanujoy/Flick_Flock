...
class Recommendation(db.Model, SerializerMixin):
    ...
    movie_series = db.relationship("MovieSeries", back_populates="recommendations")

    def __repr__(self):
        return (
            f"<Recommendation id={self.id} user_id={self.user_id} "
            f"movie_series_id={self.movie_series_id} reason={self.recommended_reason} created_at={self.created_at}>"
        )